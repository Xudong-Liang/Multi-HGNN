...
def process_data():
    ...
    count = len(metabolite)
    hg_pos = create_hypergraph(pd.read_csv('incidence_matrix_pos.csv'), count)
    hg_neg = create_hypergraph(pd.read_csv('incidence_matrix_neg.csv'), count)
    print(hg_pos)
    with open(os.path.join('data/metabolite_emb.pkl'), 'rb') as f:
    ...
    for bigg_id in bigg_ids:
        if bigg_id not in embedding_dict:
            ...
            if modified_bigg_id in embedding_dict:
                ...
                embedding = torch.tensor(np.random.rand(2304).astype(np.float32))
    ...
    return g, hg_pos, hg_neg, metabolite_features, reaction_count
...
def read_xml(xml_content):
    ...
    with open('metabolite.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['bigg_id', 'node_id'])  # 写入表头
